...
def dl_model():
    ...
    def block_2(input_tensor):
        ...
        path1 = Conv2D(filters=32, kernel_size=(3, 3), strides=(2, 2), padding='same', activation='relu')(input_tensor)

        conv_2_1 = Conv2D(filters=32, kernel_size=(1, 1), strides=(1, 1), padding='same', activation='relu')(input_tensor)
        conv_2_2 = Conv2D(filters=32, kernel_size=(3, 3), strides=(1, 1), padding='same', activation='relu')(conv_2_1)
        path2 = Conv2D(filters=32, kernel_size=(3, 3), strides=(2, 2), padding='same', activation='relu')(conv_2_2)

        path3 = MaxPooling2D(pool_size=(2, 2), strides=(2, 2), padding='same')(input_tensor)
    ...
    globalavgpool = GlobalAveragePooling2D()(block_2_output)
    output_layer = Dense(units=10, activation='softmax')(globalavgpool)
